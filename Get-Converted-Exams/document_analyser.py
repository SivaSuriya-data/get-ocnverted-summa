...
def extract_text_from_file(file_path):
    ...
    if ext == ".pdf":
        ...
        # ✅ Apply threshold to improve clarity
...
# ✏️ Rename file based on identified document
...
# 🔁 Run OCR + rename on all uploaded files
for filename in uploaded.keys():
    print(f"\n🔍 Processing: {filename}")
    try:
        text = extract_text_from_file(filename)
        ...
        print(f"✅ Renamed as: {new_filename}")
    except Exception as e:
        print(f"❌ Failed to process {filename}: {e}")

'''
# 🔁 Optimized batch processing of uploaded files
def process_uploaded_files(uploaded_files: dict) -> dict:
    ...
    for filename in uploaded_files.keys():
        ...
        try:
            ...
            if not text.strip():
                print(f"⚠️  Empty text extracted from {filename} - skipping")
                continue
            ...
            print(f"❌ OCR Error processing {filename}: {str(te)}")
        except OSError as oe:
            print(f"❌ File Operation Error with {filename}: {str(oe)}")
        except Exception as e:
